...
protocol2_vars <- c(
  ...
  "mean_stance_time_protocol2_ST_walk",
  "mean_stance_time_protocol2_ST_navigation",
  "mean_single_support_protocol2_ST_walk",
  ...
  mutate(
    variable_label = factor(
      ...
      levels = c(
        ...
        "Stance time (straight), s",
        "Stance time (navigation), s",
        "Single support (straight), s",
        "Single support (navigation), s",
...
Directly contrasting high vs low performing clusters did not reveal significant differences in BA46 activity. However, examining clusters individually did reveal some patterns.
...
results_protocol_2_contrast <- read.csv("../data/results_protocol_2_contrast.csv")
results_protocol_2_contrast <- results_protocol_2_contrast %>%
  filter(ROI == "BA46") %>%
  select(-type, -q) %>%
  rename(Condition = Contrast) %>%
  mutate(
...
results_protocol_3_contrast <- read.csv("../data/results_protocol_3_contrast.csv")
results_protocol_3_contrast <- results_protocol_3_contrast %>%
  filter(ROI == "BA46") %>%
  select(-type, -q) %>%
  rename(Condition = Contrast) %>%
  mutate(
...
format_results_table(results_protocol_2_contrast)
...
format_results_table(results_protocol_3_contrast)
...
subject_data_pd$mdsupdrs3_hy <- factor(subject_data_pd$mdsupdrs3_hy, levels = 1:4)
